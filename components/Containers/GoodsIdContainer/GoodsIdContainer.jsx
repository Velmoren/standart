	...
	useEffect(() => {
		if (ID) {
			...
			boltServices
				...
				.then((data) => {
					setFilters([
						data.data.Filters[0].mat,
						data.data.Filters[1].cov,
						data.data.Filters[2].form,
						data.data.Filters[3].dimA,
						data.data.Filters[4].dimB,
						data.data.Filters[5].pack,
					]);
				})
				.catch((e) => {
					console.error(e);
			...
			if (!isFilter) {
				boltServices
					.getItems(ID, currentPage, countPage)
					.then((data) => {
						setPic1(
		...
		if (isFilter) {
			boltServices
				...
				.then((data) => {
					...
					setNotGoods(false);
					addGoodsToCart(data.data.list);
					setSize(Math.ceil(data.data.totalsize / countPage));
					setLoading(false);
				})
				.catch((e) => {
					console.error(e);
